package main

import (
	"encoding/json"
	"log/slog"
	"net"
	"talk/common/consts"
	"talk/
	"time"
)

var userConn = make(map[string]net.Conn)
var connUser = make(map[net.Conn]string)

// cache 键是消息接收方，值是消息
var cache = make(map[string][]model.Chat)

func main() {
	listener, err := net.Listen("tcp", ":82")
	if err != nil {
		slog.Error("listen error", "error", err)
		return
	}

	for {
		conn, err := listener.Accept()
		if err != nil {
			slog.Error("accept error", "error", err)
			continue
		}
		go handleConn(conn)
	}
}

func handleConn(conn net.Conn) {
	slog.Info("new connection", "remote address", conn.RemoteAddr())
	defer func(conn net.Conn) {
		slog.Info("connection closed", "remote address", conn.RemoteAddr())
		_ = conn.Close()
	}(conn)

	buf := make([]byte, 1024)
	for {
		n, err := conn.Read(buf)
		if err != nil {
			slog.Error("客户端断开连接，清除连接映射", "error", err)
			delete(userConn, connUser[conn])
			delete(connUser, conn)
			return
		}
		msg := model.Msg{}
		err = json.Unmarshal(buf[:n], &msg)
		if err != nil {
			slog.Error("json unmarshal error", "error", err)
			return
		}
		clear(buf)
		go handleMsg(msg, conn)
	}
}

func handleMsg(msg model.Msg, conn net.Conn) {
	switch msg.MsgType {
	case consts.LoginMsgType:
		loginSt := model.Login{}
		err := json.Unmarshal(msg.Data, &loginSt)
		if err != nil {
			slog.Error("json unmarshal error", "error", err)
			return
		}
		connUser[conn] = loginSt.MyName
		userConn[loginSt.MyName] = conn

		// 把缓存的消息发送给用户
		slog.Info("要发给用户", loginSt.MyName, "的消息:", cache[loginSt.MyName])
		for _, v := range cache[loginSt.MyName] {
			sendMsg(v.MyName, v.SendTime, v.Data, conn)
			cache[loginSt.MyName] = cache[loginSt.MyName][1:]
			time.Sleep(500 * time.Millisecond)
		}
	case consts.ChatMsgType:
		chatSt := model.Chat{}
		err := json.Unmarshal(msg.Data, &chatSt)
		if err != nil {
			slog.Error("json unmarshal error", "error", err)
			return
		}

		var to string
		if chatSt.MyName == "rabbit" {
			to = "bear"
		} else {
			to = "rabbit"
		}

		toConn, ok := userConn[to]
		if !ok {
			slog.Error("user not found", "username", to)
			sendMsg("系统", time.Now().Format("15:04"), "对方不在线", conn)
			cache[to] = append(cache[to], chatSt)
			slog.Info("缓存消息", "username", to, "message", cache[to])
			return
		}

		_, _ = toConn.Write(msg.Data)

	default:
	}
}

func sendMsg(sender, sendTime, data string, conn net.Conn) {
	chatSt := model.Chat{
		Data:     data,
		SendTime: sendTime,
		MyName:   sender,
	}

	msg, err := json.Marshal(chatSt)
	if err != nil {
		slog.Error("json marshal error", "error", err)
		return
	}
	_, err = conn.Write(msg)
	if err != nil {
		slog.Error("write error", "error", err)
		return
	}
}
